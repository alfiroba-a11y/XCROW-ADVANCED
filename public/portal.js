(() => {
  ...
  function secureAdminForm() {
    ...
    form.dataset.adminSecure = '1'; form.addEventListener('submit', async event => { event.preventDefault(); event.stopImmediatePropagation(); const message = form.querySelector('.message'); message.textContent = 'Signing in securely…'; message.style.color = '#15803d'; try { const payload = Object.fromEntries(new FormData(form)); const response = await fetch('/api/auth/admin-login', { method:'POST', headers:{ 'Content-Type':'application/json' }, body:JSON.stringify(payload) }); const data = await response.json(); if (!response.ok) throw new Error(data?.error || 'Administrator sign-in failed.'); localStorage.setItem('xcrow_token', data.token); localStorage.setItem('xcrow_user', JSON.stringify(data.user)); location.reload(); } catch (error) { message.textContent = error.message; message.style.color = '#b91c1c'; } }, true);
  }
  ...
  setInterval(() => { if (location.hash === '#admin') { if (!document.querySelector('#admin-view')?.dataset.portalReady) renderAdmin(); addAdminTickets(); } addTrcRequest(); addLiveSupport(); renameWallet(); customizeReceiptButton(); correctAdminLoginCopy(); secureAdminForm(); showStkApprovalFlow(); }, 500);

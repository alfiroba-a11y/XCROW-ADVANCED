(() => {
  ...
  function correctAdminLoginCopy() {
    ...
    if (note) { note.textContent = 'Use an account you already created with an email listed in Render ADMIN_EMAILS.'; const signup = document.querySelector('#login [data-switch="signup"]'); if (signup?.closest('p')) signup.closest('p').hidden = false; }
  }
  ...
  setInterval(() => { if (location.hash === '#admin') { if (!document.querySelector('#admin-view')?.dataset.portalReady) renderAdmin(); addAdminTickets(); } addTrcRequest(); addLiveSupport(); renameWallet(); customizeReceiptButton(); correctAdminLoginCopy(); showStkApprovalFlow(); }, 500);

(() => {
    ...
    try {
      ...
      panel.innerHTML = `<span class="tag">XCROW OPERATIONS</span><h2>Full site oversight</h2>
        <div style="display:grid;grid-template-columns:repeat(4,minmax(120px,1fr));gap:10px;margin-bottom:22px">
          <div class="fee-preview"><b>${data.users}</b><br><small>Registered members</small></div><div class="fee-preview"><b>${data.escrows}</b><br><small>All escrows</small></div><div class="fee-preview"><b>${data.active}</b><br><small>Active / review</small></div><div class="fee-preview"><b>${data.trc20Pending.length}</b><br><small>TRC20 pending</small></div>
        </div>
  ...
  setInterval(() => { if (location.hash === '#admin') renderAdmin(); addTrcRequest(); }, 500);
})();
